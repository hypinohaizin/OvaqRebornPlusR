...
public class LookupManager implements Globals
{
    ...
    private static final Map<UUID, Map<String, String>> LOOKUPS_HISTORY = new HashMap<>();
    ...
    public String getNameFromUUID(UUID uuid)
    {
        ...
        new Thread(() -> {
            ...
            try
            {
                ...
                String result = jsonObject.get("username").toString().replace("\"", "");
                LOOKUPS_NAME.put(uuid, result);
            }
            catch (IOException e)
            {
                e.printStackTrace();
    ...
    {
        if (LOOKUPS_HISTORY.containsKey(uuid))
        {
            return LOOKUPS_HISTORY.get(uuid);
        }
        new Thread(() -> {
            ...
            LOOKUPS_HISTORY.put(uuid, result);
        }).start();
        return null;
    }
    ...
    {
        final String cacheKey = playerName.toLowerCase();
        // if (STATS_CACHE.containsKey(cacheKey)) return STATS_CACHE.get(cacheKey);
        ...
        new Thread(() -> {
            ...
            try
            {
                ...
                // STATS_CACHE.put(cacheKey, result);
            }
            catch (Exception e)
            {
                e.printStackTrace();
    ...
    public String get2b2tQueueSize()
    {
        new Thread(() -> {
            try
            {
                ...
                String formatted = String.format("§7Priority: §f%d, §7Regular: §f%d",
                        jsonObject.get("prio").getAsInt(), jsonObject.get("regular").getAsInt());
                // QUEUE_CACHE = formatted;
            }
            catch (Exception e)
            {
                e.printStackTrace();
