...
public class HUDModule extends ToggleModule
{
    ...
    //
    Config<WatermarkMode> watermarkModeConfig = register(new EnumConfig<>("Watermark", "Watermark style", WatermarkMode.OVAQ, WatermarkMode.values()));
    ...
    Config<Boolean> textRadarConfig = register(new BooleanConfig("TextRadar", "Displays a text‐based radar of nearby players", false));
    ...
    Config<Boolean> potionIcons = register(new BooleanConfig("PotionIcons","Displays potion icons next to potion-effect text",true, () ->potionEffectsConfig.getValue()));
    ...
    Config<ArraylistMode> arraylistModeConfig = register(new EnumConfig<>("Arraylist-Mode","Arraylist render style", ArraylistMode.OVAQ, ArraylistMode.values(), () -> arraylistConfig.getValue()));
    ...
    private List<PotionData.PotionEntry> potionEntries = new ArrayList<>();
    ...
    public void onRenderOverlayPost(RenderOverlayEvent.Post event)
    {
        ...
        if (mc.player != null && mc.world != null)
        {
            ...
            if (watermarkModeConfig.getValue() == WatermarkMode.OLD) {
                RenderManager.renderText(event.getContext(), String.format("%s §f%s (%s-%s)",
                                OvaqRebornPlusMod.MOD_NAME,  OvaqRebornPlusMod.MOD_VER,BuildConfig.BUILD_IDENTIFIER,
                                !BuildConfig.HASH.equals("null") ? "" + BuildConfig.HASH : ""),
                        2.0f, topLeft, getHudColor(rainbowOffset));
                topLeft += RenderManager.textHeight();
            } else if (watermarkModeConfig.getValue() == WatermarkMode.OVAQ) {
                renderOvaqWatermark(event.getContext(), 2.0f, topLeft);
                topLeft += RenderManager.textHeight();
            }

            topLeft += RenderManager.textHeight();

            if (textRadarConfig.getValue() && mc.world != null && mc.player != null) {
                int counter = 0;
                float radarY = topLeft;

                for (PlayerEntity other : mc.world.getPlayers()) {
                    if (other == mc.player)
                        continue;

                    String text = PlayerUtil.getColoredHealth(other) + " ";
                    String name = other.getName().getString();
                    double dist = mc.player.distanceTo(other);

                    if (Managers.SOCIAL.isFriend(name)) {
                        text += Formatting.AQUA + name + Formatting.BLUE + " " + (int) Math.round(dist);
                    } else {
                        text += Formatting.RESET + name + " " + PlayerUtil.getColoredDistance(other) + (int) Math.round(dist);
                    }

                    int pop = Managers.TOTEM.getTotems(other);
                    if (pop > 0) {
                        text += Formatting.DARK_PURPLE + " -" + pop;
                    }

                    RenderManager.renderText(event.getContext(), text, 2.0F, radarY, getHudColor(counter)
                    );

                    radarY += RenderManager.textHeight();
                    counter++;
                }
            }
            ...
            if (potionEffectsConfig.getValue()) {
                for (Map.Entry<PotionData, Animation> e1 : hudRenderPotions.entrySet()) {
                    ...
                    StatusEffect effect = e.getType().value();

                    if (effect == StatusEffects.NIGHT_VISION)
                        continue;

                    Sprite sprite = potionIcons.getValue() ? mc.getStatusEffectSpriteManager().getSprite(e.getType()) : null;

                    boolean infinite = e.getDuration() == -1;
                    boolean amplifier = e.getAmplifier() + 1 > 1 && !infinite;
                    String duration = infinite
                            ? "Inf"
                            : StringHelper.formatTicks(MathHelper.floor((float) e.getDuration()), mc.world.getTickManager().getTickRate());
                    ...
                    float xOffset = (width + 1.0f) * (float) animation.getFactor();
                    float yPos = renderingUp ? bottomRight : topRight;

                    if (sprite != null) {
                        int size = RenderManager.textHeight();
                        event.getContext().drawSprite((int) (res.getScaledWidth() - xOffset - size - 3), (int)  yPos, 0, size, size, sprite);
                    }
                    ...
                    RenderManager.renderText(event.getContext(), text, res.getScaledWidth() - xOffset, yPos, ColorUtil.fixTransparency(potionColor, (float) animation.getFactor()));

                    if (renderingUp) bottomRight -= RenderManager.textHeight();
                    else             topRight    += RenderManager.textHeight();
    ...
    private void renderOvaqWatermark(DrawContext ctx, float x, float y) {
        String mod = OvaqRebornPlusMod.MOD_NAME;
        String ver = OvaqRebornPlusMod.MOD_VER;
        String user = mc.getSession().getUsername();
        String build = BuildConfig.BUILD_IDENTIFIER2;
        String address = mc.getCurrentServerEntry() != null ? mc.getCurrentServerEntry().address : "SinglePlayer";

        int modColor = getHudColor(rainbowOffset);
        int wColor = 0xFFFFFFFF;
        int separatorColor = 0x80FFFFFF;

        float curX = x;

        RenderManager.renderText(ctx, mod, curX, y, modColor);
        curX += RenderManager.textWidth(mod);

        RenderManager.renderText(ctx, " | ", curX, y, separatorColor);
        curX += RenderManager.textWidth(" | ");

        RenderManager.renderText(ctx, ver, curX, y, wColor);
        curX += RenderManager.textWidth(ver);

        RenderManager.renderText(ctx, " | ", curX, y, separatorColor);
        curX += RenderManager.textWidth(" | ");

        RenderManager.renderText(ctx, user, curX, y, wColor);
        curX += RenderManager.textWidth(user);

        RenderManager.renderText(ctx, " | ", curX, y, separatorColor);
        curX += RenderManager.textWidth(" | ");

        RenderManager.renderText(ctx, build, curX, y, wColor);
        curX += RenderManager.textWidth(build);

        RenderManager.renderText(ctx, " | ", curX, y, separatorColor);
        curX += RenderManager.textWidth(" | ");

        RenderManager.renderText(ctx, address, curX, y, wColor);
    }
    ...
    public class HudRenderModule
    {
        ...
        public void draw(DrawContext ctx, long drawnCount)
        {
            String text   = getFormattedModule(module);
            int    width  = RenderManager.textWidth(text);
            int    height = RenderManager.textHeight();

            float xTxt = mc.getWindow().getScaledWidth() + (float) this.x;
            float yTxt = renderingUp ? topRight : bottomRight;

            int color = ColorUtil.fixTransparency(getHudColor(drawnCount - rainbowOffset), (float) module.getAnimation().getFactor());

            if (arraylistModeConfig.getValue() == ArraylistMode.PACKET)
            {
                MatrixStack m = ctx.getMatrices();

                RenderManager.rect(m, xTxt - 2,yTxt - 1,width + 4,height - 1,ColorUtil.withAlpha(0xAAAAAA, 120));
                RenderManager.rect(ctx.getMatrices(),xTxt + width + 1,yTxt - 1,1,height - 1, color);
            }

            RenderManager.renderText(ctx, text, xTxt, yTxt, color);
